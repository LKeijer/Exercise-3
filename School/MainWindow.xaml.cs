    ...
    public partial class MainWindow : Window
    {
        ...
        private void studentsList_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.Key)
            {
                ...
                case Key.Delete:
                    MessageBox.Show(e.Key.ToString() + " Was pressed", "Button Pressed: ");
                    Student selectedStudent = this.studentsList.SelectedItem as Student;
                    MessageBoxResult result = MessageBox.Show(string.Format("Remove {0}", selectedStudent.FirstName + " " + selectedStudent.LastName), "Confirm", MessageBoxButton.YesNo, MessageBoxImage.Question, MessageBoxResult.No); // what does the MessageBoxResult.No do? Seems redundant, does the same whether it is included or not.
